...
it('should return an account', async () => {
    const coinbaseClient = new CoinbaseClient(createRequestMaker(accountDataProvider))
...
it('should get exchange rates', async () => {
    const coinbaseClient = new CoinbaseClient(createRequestMaker(exchangeRateDataProvider))
...
it('should create an address', async () => {
    const coinbaseClient = new CoinbaseClient(createRequestMaker(addressDataProvider))
...
it('should get a list of transactions', async () => {
    const coinbaseClient = new CoinbaseClient(createRequestMaker(paginatedTransactionResponseDataProvider))
...
it('should get a transaction', async () => {
    const coinbaseClient = new CoinbaseClient(createRequestMaker(transactionDataProvider))
...
it('should throw exception on send money when toFinancialInstitution is set and financialInstitutionWebsite not', async () => {
    ...
    expect(expectedException?.message).toStrictEqual('financialInstitutionWebsite is required when toFinancialInstitution is true')
})

it('should transfer money', async () => {
    const coinbaseClient = new CoinbaseClient(createRequestMaker(transactionDataProvider))
    const response = await coinbaseClient.transferMoney('abc', '123', '0.01', 'BTC')
    expect(response).toStrictEqual(transactionDataProvider);
})

it('should show payment method', async () => {
    const coinbaseClient = new CoinbaseClient(createRequestMaker(paymentMethodDataProvider))
...
it('should show payment methods', async () => {
    const coinbaseClient = new CoinbaseClient(createRequestMaker(paginatedPaymentMethodDataProvider))
    const response = await coinbaseClient.showPaymentMethod('abc')
    expect(response).toStrictEqual(paginatedPaymentMethodDataProvider);
})

it('should get fiat currencies', async () => {
    const coinbaseClient = new CoinbaseClient(createRequestMaker(fiatCurrencyDataProvider))
...
it('should get a time', async () => {
    const coinbaseClient = new CoinbaseClient(createRequestMaker(timeDataProvider))
